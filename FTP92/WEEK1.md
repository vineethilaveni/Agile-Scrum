...
IdentityFile ~/.ssh/id_rsa
  ...
  ** Clone repository at Gitbash **
    ...
    * `pwd` -- Check and verify that the current working directory is C:\users\Hvuser/workspace
    * `git clone ssh://git-codecommit.us-east-2.amazonaws.com/v1/repos/ftp92 `
    * `cd ftp92`
    * `git status`
  * open Visual Studio (VS) Code
    * Open folder c:\users\Hvuser\workspace\ftp92
...
# Workshop material - Day #2 - MySQL Workbench/Cli/Curl/Browser to interact w/ application
  ...
  * `CREATE DATABASE ftp92;` and click the lightning button
  * `CREATE USER 'ftp92'@'localhost' IDENTIFIED BY 'ftp92';`
  * `GRANT ALL ON ftp92.* TO 'ftp92'@'localhost';`
  ...
  * go to gitbash, ensure you are in workspace/ftp92
  ...
  * Build the war file as follows:
    ...
    * `cp target/ftp92-0.0.1-SNAPSHOT.war /D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/ftp92.war`
    * start tomcat and tail its logs
      * `cd D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16`
      * `rm -rf logs/*`
      * `./bin/startup.sh`
      * `tail -f logs/*`
      * `curl -vvv http://localhost:8080/ftp92/api/employees | python -m json.tool`
      * `curl -vvv http://localhost:8080/ftp92/api/employees/2000 | python -m json.tool`
   * `cd ../../webui/lm-app/`
   * `npm install`
   * `ng build`
   * `cp ../../restservice/leavemanager/target/ftp92-0.0.1-SNAPSHOT.war ./ftp92.war`
   * `cd dist`
   * `jar -uvf ../ftp92.war *`
   * `cd ..`
   * `jar -tvf ftp92.war`
   * `cp ftp92.war /D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/ftp92.war`
   * Notice that the tail terminal shows that the new version of the web application archive is now getting deployed
   * Open Chrome browser and navigate to http://localhost:8080/ftp92/
...
# Workshop material - Day #4 - Configure Project Board in Trello for  Managing and Tracking Your Project
  ...
  * Create a board for your project as "ftp92"
  ...
  * [Team member #3]
     * Create "Current Sprint" with "ftp92 Sprint 1" as label 
